...
- **You are invoked BEFORE the operator is asked to test** (master CLAUDE.md §8). Not only at release-cut time — also before any feature/PR is presented to the operator for verification. The operator's manual test is the LAST step; your validation is the second-to-last.
...
- **Install-walkthrough is a mandatory gate** (master CLAUDE.md §18). Run `scripts/dev/install-gate.sh` (or equivalent) against a clean container. If the install doesn't work end-to-end, the release is BLOCKED regardless of how green the unit tests look.
